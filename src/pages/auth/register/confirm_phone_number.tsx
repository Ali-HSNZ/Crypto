    ...
    const validationSchema = Yup.object({
        password: Yup.string()
            ...
            .test('validate', 'رمز عبور معتبر نیست | رمز عبور میتواند ترکیبی از عدد و حروف انگلیسی باشد.', (values) => {
                if (VALIDATION_PASSWORD.test(toEnDigits(values))) return true
                else return false
            }),
    })
                ...
                >
                    ...
                    <section className='flex w-full lg:w-[700px] flex-col mt-6 flex-1 px-6 sm:px-14 py-6 '>
                        ...
                        <div className='flex items-center rounded-full mt-4 bg-[#E8F4FF] py-4 px-6 gap-x-4'>
                            ...
                            <p className='text-xs font-iranyekan-regular leading-6'>
                                کد تائید به شماره {toPersianDigits(phone)} ارسال شده است. این کد تا{' '}
                                {toPersianDigits(formatTime(remainingTime))} دقیقه دیگر معتبر است
                            </p>
                        </div>
                        ...
                        <div className='w-full flex flex-row-reverse justify-center items-center mt-3'>
                            <OTPInput
                                ...
                                renderInput={(props) => (
                                    <input
                                        {...props}
                                        disabled={isOtpVerificationStatus === true}
                                        className={`border-2 otp_inputs ${
                                            isOtpVerificationStatus === true
                                                ? 'border-green-400'
                                                : isOtpVerificationStatus === false
                                                  ? 'border-red-300'
                                                  : 'border-gray-300'
                                        } rounded-xl font-iranyekan-bold text-gray-700`}
                                    />
                                )}
                            />
                        </div>
                        <div className='w-full flex justify-center items-center flex-col '>
                            <button
                                type={'button'}
                                disabled={isOtpVerificationStatus === true}
                                onClick={checkOtpValidity}
                                className={`disabled:bg-gray-400 disabled:cursor-not-allowed bg-blue-600 hover:bg-blue-700 duration-150 font-iranyekan-bold text-blue-100 w-fit px-6 py-3 rounded-xl mt-4`}
                            >
                                تایید شماه همراه
                            </button>
                    ...
                    <section className='w-full p-5 flex justify-between items-center'>
                        ...
                        <button
                            type={'submit'}
                            disabled={!isOtpVerificationStatus}
                            className={`${
