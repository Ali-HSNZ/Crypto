...
import { type TInputProps } from '../core/types/other.types'
        ...
        <div className='relative w-full flex flex-col rounded-full '>
            ...
            <input
                ...
                className={`font-iranyekan-regular ${
                    formik.errors[name] && formik.touched[name]
                        ...
                        : 'focus:border-gray-300 bg-white hover:border-gray-300 border-gray-200'
                } placeholder:text-sm placeholder:text-gray-400 ${
                    disabled && 'cursor-not-allowed'
                }  pl-11 pr-[70px] border-2 w-full py-4 rounded-full outline-none`}
            />

            {/* Icon */}
            <label
                htmlFor={`${name}_input`}
                className='absolute cursor-pointer top-0 right-0 pr-6 px-2 py-[17px] rounded-r-full'
            >
                {icon}
            </label>
            ...
            {type === 'password' && (
                <button
                    ...
                    className='absolute top-0 left-0 pl-4 pr-2 bottom-0 rounded-l-full text-gray-400 hover:text-gray-500'
                >
                    {isPasswordVisible ? (
                        <svg
