...
const InputCommon : FC<TInputCommonProps> = ({icon , disabled , inputType , title  , placeholder , children , childrenParentHover , value}) : JSX.Element => {
     ...
     return (  
          <div className="relative w-full rounded-full ">
               ...
               {/* input */}
               <input disabled={disabled ?? false} ref={inputRef}  placeholder={placeholder} type={inputType}  className='font-iranyekan-regular placeholder:text-sm placeholder:text-gray-400 focus:border-gray-300 hover:border-gray-300 border-gray-200 pl-11 pr-[70px] border-2 w-full py-4 rounded-full outline-none' />
               {/* Icon */}
               ...
               {/* children */}
               {inputType !== 'password' && <div className='absolute top-0 left-0 pl-3 flex items-center justify-center pr-2 bottom-0 rounded-l-full'>{children}</div>}
          </div>
     );
}
