...
type TInputCommonProps = {
     ...
     dir? : 'rtl' | 'ltr',
     placeholder? : string,
     inputType :   'password' | 'tel' | 'text' | "email"
...
const InputCommon : FC<TInputCommonProps> = ({icon , disabled , inputType , title , maxLength , placeholder , children , formik , name}) : JSX.Element => {
     ...
     return (  
          <div className="relative w-full flex flex-col rounded-full ">
               ...
               {inputType === 'password' && <button type={'button'} onClick={changeInputType} className='absolute top-0 left-0 pl-4 pr-2 bottom-0 rounded-l-full text-gray-400 hover:text-gray-500'>
                    {isHiddenPassword ? (
